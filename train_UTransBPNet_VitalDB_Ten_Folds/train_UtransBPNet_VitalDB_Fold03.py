...
def train_model(model, train_loader, val_loader, optimizer, criterion=nn.L1Loss(), num_epochs=100, patience=7,
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            train_mae = np.mean(np.abs(true_data - pred_data))
            val_mae = test_model(model, val_loader)
        print(
            f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {train_mae:.6f}, Validation Loss: {val_mae:.6f}, Best Loss: {best_val_loss:.6f}")

        if val_mae < best_val_loss:
            print(
                f'Validation loss decreased ({best_val_loss:.6f} --> {val_mae:.6f}).  Saving model ...'
            )
...
weight_path = "weight_model/UtransBPNet_VitalDB_Fold03_"
