...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, group_width=1,
        ...
        self.conv2 = conv3x3(width, width, stride, width //
                             min(width, group_width), dilation)
...
class Stage(nn.Module):
    ...
    def __init__(self, block, inplanes, planes, group_width, blocks, stride=1, dilate=False, cheap_ratio=0.5):
        ...
        raw_planes = int(planes * (1 - cheap_ratio) /
        ...
        self.merge = nn.Sequential(
            ...
            nn.Conv1d(planes + raw_planes * (blocks - 2), cheap_planes,
                      kernel_size=1, stride=1, bias=False),
            nn.BatchNorm1d(cheap_planes),
            nn.ReLU(inplace=True),
            nn.Conv1d(cheap_planes, cheap_planes, kernel_size=1, bias=False),
            nn.BatchNorm1d(cheap_planes),
        )
        self.cheap = nn.Sequential(
            nn.Conv1d(cheap_planes, cheap_planes,
                      kernel_size=1, stride=1, bias=False),
            nn.BatchNorm1d(cheap_planes),
        )
...
class GGhostRegNet(nn.Module):
    ...
    def __init__(self, block, layers, widths, num_classes=625, zero_init_residual=True,
        ...
        self.conv1 = nn.Conv1d(4, self.inplanes, kernel_size=3, stride=2, padding=1,
                               bias=False)
...
if __name__ == '__main__':
    model = g_ghost_regnetx_006()
    model.eval()
    # print(model)
    input = torch.randn(32, 4, 625)
    y = model(input)
    print(y.size())
