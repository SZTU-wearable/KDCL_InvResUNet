...
class ConvBnAct(nn.Module):
    def __init__(self, in_chs, out_chs, kernel_size,
        ...
        self.conv = nn.Conv1d(in_chs, out_chs, kernel_size, stride, kernel_size // 2, bias=False)
        self.bn1 = nn.BatchNorm1d(out_chs)
        self.act1 = act_layer(inplace=True)
...
class GhostModuleV2(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1, relu=True, mode=None, args=None):
        ...
        if self.mode in ['original']:
            ...
            self.short_conv = nn.Sequential(
                # nn.Conv1d(inp, oup, kernel_size, stride, kernel_size // 2, bias=False),
                # nn.BatchNorm1d(oup),
                # nn.Conv1d(oup, oup, kernel_size=(1, 5), stride=1, padding=(0, 2), groups=oup, bias=False),
                # nn.BatchNorm1d(oup),
                # nn.Conv1d(oup, oup, kernel_size=(5, 1), stride=1, padding=(2, 0), groups=oup, bias=False),
                # nn.BatchNorm1d(oup),

                nn.Conv1d(inp, oup, kernel_size, stride, kernel_size // 2, bias=False),
                nn.BatchNorm1d(oup),
                nn.Conv1d(oup, oup, kernel_size=1, stride=1, padding=0, groups=oup, bias=False),
                nn.BatchNorm1d(oup),
                nn.Conv1d(oup, oup, kernel_size=5, stride=1, padding=2, groups=oup, bias=False),
                nn.BatchNorm1d(oup),
            )
    ...
    def forward(self, x):
        if self.mode in ['original']:
            ...
            res = self.short_conv(F.avg_pool1d(x, kernel_size=2, stride=2))
            x1 = self.primary_conv(x)
            x2 = self.cheap_operation(x1)
            out = torch.cat([x1, x2], dim=1)
            return out[:, :self.oup, :] * F.interpolate(self.gate_fn(res), size=out.shape[-1],
                                                           mode='nearest')
...
class GhostBottleneckV2(nn.Module):
    ...
    def __init__(self, in_chs, mid_chs, out_chs, dw_kernel_size=3,
        ...
        if self.stride > 1:
            self.conv_dw = nn.Conv1d(mid_chs, mid_chs, dw_kernel_size, stride=stride,
        ...
        if (in_chs == out_chs and self.stride == 1):
            ...
            self.shortcut = nn.Sequential(
                nn.Conv1d(in_chs, in_chs, dw_kernel_size, stride=stride,
...
class GhostNetV2(nn.Module):
    def __init__(self, cfgs, num_classes=1000, width=1.0, dropout=0.2, block=GhostBottleneckV2, args=None):
        ...
        self.conv_stem = nn.Conv1d(4, output_channel, 3, 2, 1, bias=False)
        ...
        self.conv_head = nn.Conv1d(input_channel, output_channel, 1, 1, 0, bias=True)
...
def ghostnetv2(num_classes=625):
    cfgs = [
        # k, t, c, SE, s
        # Dw_kernel_size, Expansion_size, Out_size, SE, Stride
        [[3, 16, 16, 0, 1]],
        [[3, 48, 24, 0, 2]],
        [[3, 72, 24, 0, 1]],
        [[5, 72, 40, 0.25, 2]],
