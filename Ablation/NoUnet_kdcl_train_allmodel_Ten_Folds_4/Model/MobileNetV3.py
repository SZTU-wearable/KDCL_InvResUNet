...
class MobileNetV3(nn.Module):
    def __init__(self,
        ...
        layers.append(ConvBNActivation(4,
                                       firstconv_output_c,
                                       kernel_size=3,
                                       stride=2,
                                       norm_layer=norm_layer,
        ...
        # print(lastconv_output_c, last_channel)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # x = self.features(x)
        for sub_module in self.features:
            # print(x.shape)
            x = sub_module(x)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
...
def mobilenet_v3_large(num_classes: int = 625,
...
def mobilenet_v3_small(num_classes: int = 1000,
    ...
    # print(reduce_divider, last_channel)
    return MobileNetV3(inverted_residual_setting=inverted_residual_setting,
                       last_channel=last_channel,
                       num_classes=num_classes)
